...
Você pode criar cards no dashboard do Home Assistant para visualizar os alertas meteorológicos. Clique em qualquer alerta para ver todos os detalhes (descrição, severidade, datas, riscos e instruções).

### Opção 1: Card de Entidades

```yaml
type: entity
entity: sensor.inmet_alert_boituva_sp
grid_options:
  columns: 12
  rows: 2
...
### Opção 2: Card Avançado com Auto-Entities

Para listar dinamicamente todos os alertas ativos, instale via HACS → Frontend:
- `auto-entities` 
- `button-card`

Depois use este código YAML:
